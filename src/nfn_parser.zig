...
pub const NfnParser = struct {
    ...
    fn parse(self: *NfnParser) !void {
        const sub_id_col = self.csv_parser.firstInRow(0, @constCast("subject_ids")).?;
        const class_id_col = self.csv_parser.firstInRow(0, @constCast("classification_id")).?;
        const user_name_col = self.csv_parser.firstInRow(0, @constCast("user_name")).?;
        const anno_col = self.csv_parser.firstInRow(0, @constCast("annotations")).?;

        for (1..self.csv_parser.rows) |i| {
            const sub_id = self.csv_parser.get(i, sub_id_col).?;
            const class_id = self.csv_parser.get(i, class_id_col).?;
            const user_name = self.csv_parser.get(i, user_name_col).?;
            const annos = self.csv_parser.get(i, anno_col).?;
